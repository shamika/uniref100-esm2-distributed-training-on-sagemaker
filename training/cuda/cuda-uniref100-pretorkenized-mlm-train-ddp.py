...
def get_index_file_index_path(base_path, folder):
    index_file_path = os.path.join(base_path, folder)
    print("Index file path is [{}]".format(index_file_path))
    csv_files = [file for file in listdir(index_file_path) if file.endswith('.csv')]
    print("Index file location is [{}]".format(csv_files[0]))
    return csv_files[0]
...
if __name__ == "__main__":
    ...
    train_dataset = Uniref100TorkenizedDataset(args.training_dir, get_index_file_index_path(args.training_dir, "train_index_map"))
    test_dataset = Uniref100TorkenizedDataset(args.test_dir, get_index_file_index_path(args.training_dir, "test_index_map"))
