...
class CustomDataset(torch.utils.data.Dataset):
    ...
    def __init__(self, list_IDs, file_index_map_df, data_dir):
    ...
    def __getitem__(self, index):
        ...
        #print(f"Dataset GetItem completed in {timer() - run_start} sec.")
...
def get_file_index_map(args):
    ...
    #TODO : Does this need to be done by each GPU instance ?
    ...
    print("Created the file index map. with shape : ")
    print(file_index_map_df.shape)
    print(f"File map creation completed in {timer() - run_start} sec.")
...
if __name__ == "__main__":
    ...
    train_dataset = CustomDataset(train_partition, file_index_map_df, args.training_dir)

    test_partition = list(range(nTrain,num_lines+1))
    test_dataset = CustomDataset(test_partition, file_index_map_df, args.training_dir)
    ...
    for epoch in range(starting_epoch, args.num_epochs):
        ...
        model.train()
        run_start_batch = timer()
        
        for idx, batch in enumerate(train_loader):
            
            print(f"Retrival of batch completed in {timer() - run_start_batch} sec.")
